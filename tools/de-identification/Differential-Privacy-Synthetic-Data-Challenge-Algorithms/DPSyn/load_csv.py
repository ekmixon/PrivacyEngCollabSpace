...
class LoadCSV:
    ...
    def load_original_records(self, dataset_path):
        ...
        self.logger.info(f"loaded {self.encoded_records.shape[0]} records")
    ...
    def load_dataset_parameters(self, dataset_path, json_path):
        ...
        # read code mapping
        self.code_mapping = pickle.load(
            open(f"{config.MAPPING_PATH}code_mapping", "rb")
        )
    ...
    def transform_records_to_num(self):
        ...
        for index, attribute_name in enumerate(self.dataset_header):
            self.logger.info(f"transforming attribute {attribute_name}: {index}")
