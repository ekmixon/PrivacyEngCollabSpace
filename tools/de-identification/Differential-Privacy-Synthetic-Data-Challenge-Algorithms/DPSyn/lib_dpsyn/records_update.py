...
class RecordUpdate:
    ...
    def update_records_before(self, view, view_index, iteration):
        ...
        self.logger.info(f"the l1 error before updating is {l1_error}")
    ...
    def update_records_after(self, view, view_index, iteration):
        ...
        self.logger.info(f"the l1 error after updating is {l1_error}")
    ...
    def minimum_cost_flow_problem_intuitive(self):
        ...
        marginal_reallocation[
            list(range(num_cells)), list(range(num_cells))
        ] = minimum_value
    ...
    def actual_cumulative(self, marginal_reallocation, actual_distribution, synthesize_distribution):
        while (
            np.sum(synthesize_distribution) != 0
            and np.sum(actual_distribution) != 0
        ):
