...
class ExperimentDPSyn(Experiment):
    ...
    def calculate_noise_parameter(self, num_views):
        ...
        self.logger.info(f"sigma = {self.gauss_sigma}")
    
    def construct_views(self, basis):
        view_attri = [self.dataset_header[bas] for bas in basis]

        self.logger.info(f"constructing {tuple(view_attri)}")
    ...
    def consist_views(self, views, consist_key):
        ...
        consist_views = {key: views[key] for key in consist_key}
