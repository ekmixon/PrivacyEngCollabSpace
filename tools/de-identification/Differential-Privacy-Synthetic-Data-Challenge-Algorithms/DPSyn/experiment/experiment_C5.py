...
class Experiment_C5(ExperimentDPSynMCF):
    ...
    def join_records(self, records, sl_records, attribute_index_dict):
        ...
        self.logger.warning(
            f"age indices inconsist number is {age_bin_2_remain_indices.size - age_bin_1_remain_indices.size}"
        )
    ...
    def calculate_sample_line_default(self):
        ...
        for i, attri_name in enumerate(self.sample_line_default):
            default_value[i] = (
                np.where(
                    self.code_mapping[attri_name]
                    == self.sample_line_default[attri_name]
                )[0]
                if attri_name in self.code_mapping
                else self.sample_line_default[attri_name]
            )
        ...
        for incwage_value in range(1, 7):
            ...
            if incwage_value in [1]:
                ...
                for index in minc_cell_indices:
                    view_dist.extend(
                        (
                            self.minc_marginal.count[index] * ratio[0],
                            self.minc_marginal.count[index] * ratio[1],
                        )
                    )
